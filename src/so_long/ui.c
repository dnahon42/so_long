...
/*   Updated: 2025/06/20 15:22:59 by dnahon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_map(t_data *data)
{
	...
	mlx_put_image_to_window(data->mlx, data->win, data->player, data->x,
		data->y);
	mlx_put_image_to_window(data->mlx, data->win, data->enemy, data->enemy_x,
		data->enemy_y);
}
...
void	display_ui(t_data *data)
{
	...
	free(movements);
	collectibles_left = ft_itoa(data->total_collectibles
			- data->collected_items);
	mlx_string_put(data->mlx, data->win, WIN_WIDTH - 200, 10, 0xFFFFFF,
		"Collectibles left: ");
